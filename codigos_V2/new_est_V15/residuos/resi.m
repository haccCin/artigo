...
somRef = somCanais(canRef, compo, 60, perce1); 
                                              ...
                                              % frações percentuais. 
somTotRef = somCanais(canRef, newTot, 60,perce2); 
...
iteMMQMat = 0;
for i = 1 : tamCan(1,1)
   soma = somCanais(cana(i,:), compo, 60, perce1); 
                                                   % soma canais na
                                                   % partição em estudo 
   somComp(i,:) = soma;                             
   somTotEsc = somCanais(cana(i,:), newTot, 60,perce2); 
                                                        % soma das
                                                        % contagens
                                                        % espectrais totais
                                                        % na partição em
   ...
   tamSomRef = size(somRef);
   mat = [somRef ; soma; ones(1, tamSomRef(1,2))]; 
   ...
   resid(i,1) = dif; 
   resid2 (i,1) = diAprMat;
   if dif > maxResi && ismember(cana(i,1), canRef) == 0
       maxResi = dif;
       iteMMQ = i;
   end
