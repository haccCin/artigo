...
% recuperação dos dados da partição de maior resíduo. 
...
% o laço será executado até conseguir um valor de altura válido. 
while parou == false
    % esse bloco sera executado se a atual altura for maior que a metade.  
    if presPos == 1
        % Valores reais que serão fatore multiplicativos. O primeiro é um 
        % fator multiplicativo para sair da altura atual para o máximo
        % valor. O segundo para sair da altura do retangulo ate a metade. 
        c1 = max / altRet;
        c2 = presHalf / altRet;
        % fator multiplicativo para alterar a altura atual. 
        height=c2+rand(1,1)*(c1 - c2); 
        % essa condição vai acontecer para aumentar a altura. 
        if height > 1 
            newHeig = altRet * height;
            posVarAlt = 1;
        % condição para diminuir a altura. 
        else
            newHeig = altRet * height;
            posVarAlt = 0;
        end
    ...
    % esse bloco sera executado se a atual altura for menor que a metade.
    else
        % Valores reais que serão fatore multiplicativos. O primeiro é um 
        % fator multiplicativo para sair da altura atual para a metade do
        % valor. O segundo para sair da metade para o mínimo valor. 
        c1 = presHalf / altRet;
        c2 = min / altRet;
        % fator multiplicativo para alterar a altura atual. 
        height=c2+rand(1,1)*(c1 - c2); 
