...
partRefTemp = [starWide endWide ; partRef]; % une as partições de referência 
                                        % com a partição definida no
                                        % momento. 
partRef = ordenacao(partRefTemp);                                       
...
% escrita em arquivo dos valores determinados no passo anterior. 

escrit = fopen('aarqCoorBariPoin','a');
impressao(escrit, x');
fclose(escrit);
...
esc = fopen('aarqdivCoorBari','a');
impressao(esc, divBari);
fclose(esc);
