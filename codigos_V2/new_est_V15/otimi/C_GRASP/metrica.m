function [ valor4,resul ] = metrica( est )
...
% calculando o espectro estimado:deixar comentado por enquanto. 

% qtdPart = size(est{1,1});
% contEspc = est{1,1}{3,1}.compostos;
% for i = 1 : qtdPart(1,1)
%     canIni= est{1,1}{i,1}.starWide;
%     canFin = est{1,1}{i,1}.endWide;
%     compost =  est{1,1}{3,1}.compostos;
%     toti = est{1,1}{3,1}.total;
%     for q = canIni:1:canFin
%         if est{1,1}{3,1}.compostos(q,1) ~= -1
%             conEst(q,1) = compost(q,:) * est{4,1}(:,4);
%             conRea(q,1) = toti(q,1);
%         end
%     end
% end
