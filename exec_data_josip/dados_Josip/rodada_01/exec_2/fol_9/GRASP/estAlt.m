...
partRef = ordPart([starWide endWide ; partRef]); % une as partições de referência 
...
if any(x<0) == 1
    ...
    finis = false;
    while finis == false 
        if x(control,1) < 0 || control == tamX(1,1)
            dotNeg = matSoma(:,control);
            finis = true;
        else
            control = control + 1;
        end
...
%% Determinação dos pontos na reta que tem CB negativa


ponto = matSoma * x; %coordenadas do ponto com CB negativas.
vet = ponto - dotNeg; %vetor entre projeção e ponto com CB negativas. 
vetNew = ponto - dotNeg*x(control,1) ; %vetor entre o total e ponto com CB negativas 
passo = 0.01; % variavel de controle
parou = false;% variavel de controle 
h = 1;        % variavel de controle  
while parou == false
    vetTemp = vetNew * (1-0.01*h);
    dotTemp = dotNeg + vetTemp;
    estim(:,h) = dotTemp;
    h = h + 1;
    if (1-0.01*h == 0)
        parou = true;
...
% vetor decrescido na primeira linha do laço.
% 
...
% coorComp = x;
% while parou == false
%     newVetDecres =  passo * (vetNew); % vetor ´vet´ decrescido 
%     newPontDescres = [dotNeg + newVetDecres; matUmTot]; % novo ponto
%     newCoorBari = ((mS)'* mS)^(-1) * mS' * newPontDescres;
%     newCoorBari
%     if  compBari(newCoorBari, coorComp, 0.0001) == true
%         parou = true;
%     else
...
for s = 1 : tamPercen(1,2)
   ...
   for r = 1 : tamMat(1,2)
       ...
       for p = 1 : tamLin(1,1)
            temp = abs((resul2{1,s}(p,r)) / (partRef(p,2) - partRef(p,1)));
            resul7{1,s}(p,r) = temp;
       end
   end
end
